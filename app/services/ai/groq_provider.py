"""Groq AI provider — fast classify / summarize / translate."""
...
class GroqProvider:
    ...
    async def _analyze(
        ...
        *,
        ...
        if not clipped:
            ...
            return AnalyzedPost(result=empty, meta=CallMeta(provider="groq", status="ok", operation="analyze"))
        user = (
    ...
    async def search_answer(
        ...
        return await self._search(query, contexts, operation="search")
    ...
    async def merge_news(
        ...
        return await self._search(query, contexts, operation="merge")
    ...
    async def analyze_relations(
        ...
        return await self._search(query, contexts, operation="relations")
        ...
        *,
        ...
        if not contexts:
            return AnsweredQuestion(
                ...
                meta=CallMeta(provider="groq", status="ok", operation=operation),
            )
        ...
        data, chat = await self._client.chat_json(
            system=SEARCH_SYSTEM, user=user, temperature=0.1
        )
        return AnsweredQuestion(
